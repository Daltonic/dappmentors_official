...
const Controls: React.FC<{
  ...
  setSelectedTab: (tab: "all" | Service["type"]) => void;
  statusFilter: "all" | Service["status"];
  setStatusFilter: (status: "all" | Service["status"]) => void;
  searchTerm: string;
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select
            value={statusFilter}
            onChange={(e) =>
              setStatusFilter(e.target.value as Service["status"] | "all")
            }
        ...
        {selectedServices.size > 0 && (
          <div className="flex gap-2 flex-wrap">
            <button className="px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-xl text-sm font-medium hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
              Activate Selected ({selectedServices.size})
            </button>
